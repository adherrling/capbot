...
def run_bot(capped_users, token):
    ...
    async def on_ready():
        ...
        for user in capped_users:
            ...
            user_updates += f"{user} has capped at the citadel at approximately: {curdate}.\n"
        if user_updates != "":
    ...
    async def on_message(message):
        ...
        if message.content.startswith('!caplist'):
            pass
            # capped_users = session.query(
            #     Account.name).filter(Account.capped_this_week == True).all()
            # users = []
            # for user in capped_users:
            #     users.append(user[0])
            # output = "\n".join(users)
